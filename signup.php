    ...
    <div class="flex justify-center items-center min-h-screen">
        <div class="bg-white shadow-lg rounded-lg p-8 w-full max-w-md">
            ...
            if (isset($_POST['tombolSignup'])) {
                ...
                if (empty($username) || empty($password) || empty($password1) || empty($telepon) || empty($rumah)) {
            ?><script>
                        let messageAllField = '<div class="text-red-500 mt-4 text-center">Semua field harus diisi!</div>';
                        document.getElementById('emptyPublished').innerHTML = messageAllField;
                    </script>
                <?php
                } elseif ($password !== $password1) {
                ?><script>
                        let messagePasswordConfirm = '<div class="text-red-500 mt-4 text-center">Password tidak cocok!</div>';
                        document.getElementById('emptyPublished').innerHTML = messagePasswordConfirm;
                    </script>
                    <?php
                } else {
                    ...
                    ?><script>
                            let messageUsername = '<div class="text-red-500 mt-4 text-center">Username sudah digunakan!</div>';
                            document.getElementById('emptyPublished').innerHTML = messageUsername;
                        </script>
                        <?php
                    } else {
                        $sql = "INSERT INTO tb_pelanggan (username, no_tlp, alamat, password) VALUES (?, ?, ?, ?)";
                        $stmt = $koneksi->prepare($sql);
                        $stmt->bind_param("ssss", $username, $telepon, $rumah, $hashedPassword);

                        if ($stmt->execute()) {
                        ?><script>
                                let messageDone = '<div class="text-green-500 mt-4 text-center">Pendaftaran berhasil!</div>';
                                document.getElementById('emptyPublished').innerHTML = messageDone;
                            </script>
                        <?php
                        } else {
                        ?><script>
                                let messageWrongSQL = '<div class="text-red-500 mt-4 text-center">Terjadi kesalahan. Coba lagi!</div>';
                                document.getElementById('emptyPublished').innerHTML = messageWrongSQL;
                            </script>
            <?php
                        }
                    }
